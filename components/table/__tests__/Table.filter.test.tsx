...
describe('Table.filter', () => {
  ...
  it('not show filter icon when undefined', () => {
    ...
    expect(container.querySelectorAll('.g-table-filter-column')).toHaveLength(0);
  });
  ...
  it('renders menu correctly', async () => {
    ...
    fireEvent.click(container.querySelector('span.g-dropdown-trigger')!, nativeEvent);
    await waitFor(() =>
      expect(container.querySelector('.g-table-filter-dropdown')).toMatchSnapshot(),
    );
  });
  ...
  it('should handle filterDropdown undefined correctly', () => {
    ...
    // 检查是否正确选中了Boy选项
    const boyMenuItem = container.querySelectorAll('.g-dropdown-menu-item-selected')[0];
  ...
  it('override custom filter correctly', () => {
    ...
    fireEvent.click(container.querySelector('span.g-dropdown-trigger')!);
    ...
    fireEvent.click(container.querySelector('span.g-dropdown-trigger')!);
    fireEvent.click(container.querySelector('#setSelectedKeys')!);
    expect(container.querySelector('.g-dropdown-open')).toBeTruthy();
    fireEvent.click(container.querySelector('#confirm')!);
    expect(renderSelectedKeys).toEqual([42]);
    expect(container.querySelector('.g-dropdown-open')).toBeFalsy();
  ...
  it('if the filter is visible it should ignore the selectedKeys changes', () => {
    ...
    const checkboxList = container
      ?.querySelector('.g-table-filter-dropdown')
    ...
    fireEvent.click(
      container.querySelector('.g-table-filter-dropdown')?.querySelector('input[type="checkbox"]')!,
    );
    fireEvent.click(
      container
        ?.querySelector('.g-table-filter-dropdown')
        ?.querySelector('.g-table-filter-dropdown-btns .g-btn-color-primary.g-btn-varig-solid')!,
    );
  ...
  it('fires change event when visible change', () => {
    ...
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
  ...
  it('render checked of checkbox correctly controlled by filteredValue', () => {
    ['Lucy', 23, false].forEach((val) => {
      ...
      expect(
        container
          ?.querySelector('.g-table-filter-dropdown')
          ?.querySelectorAll<HTMLInputElement>('.g-checkbox-input')[0].checked,
      ).toBe(true);
    });
  ...
  it('three levels menu', () => {
    ...
    function getFilterMenu() {
      return container.querySelector('.g-table-filter-dropdown');
    }

    // Open Level2
    fireEvent.mouseEnter(
      getFilterMenu()?.querySelectorAll('div.g-dropdown-menu-submenu-title')[0]!,
    );
    refreshTimer();

    // Open Level3
    fireEvent.mouseEnter(
      getFilterMenu()?.querySelectorAll('div.g-dropdown-menu-submenu-title')[1]!,
    );
    refreshTimer();

    // Select Level3 value
    const items = getFilterMenu()?.querySelectorAll('li.g-dropdown-menu-item');
    ...
    const latestItems = getFilterMenu()?.querySelectorAll('li.g-dropdown-menu-item');
  ...
  it('works with JSX in controlled mode', () => {
    ...
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
    fireEvent.click(container.querySelector('.g-table-filter-dropdown-btns .g-btn-link')!);
  ...
  it('reset dropdown filter correctly', () => {
    ...
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
    fireEvent.change(container.querySelector('.g-input')!, { target: { value: 'test' } });
    expect(container.querySelector<HTMLInputElement>('.g-input')?.value).toBe('test');
    fireEvent.click(container.querySelector('.g-btn')!);
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
    expect(container.querySelector<HTMLInputElement>('.g-input')?.value).toBe('');
  });
  ...
  it('should reset pagination after filter', () => {
    ...
    expect(container.querySelectorAll('.g-pagination-item')).toHaveLength(0);
  });
  ...
  it('should keep pagination current after filter', () => {
    ...
    expect(container.querySelector('.g-pagination-item-active')?.textContent).toBe('3');
    // Warning: An update to Item ran an effect, but was not wrapped in act(...).
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
    fireEvent.click(container.querySelector('.g-dropdown-menu-item')!);
    fireEvent.click(
  ...
  it('Not cache for onChange state', () => {
    ...
    // Sort it
    fireEvent.click(container.querySelector('.g-table-column-sorters')!, nativeEvent);
    ...
    onChange.mockReset();
    // Warning: An update to Item ran an effect, but was not wrapped in act(...).
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!, nativeEvent);
    fireEvent.click(container.querySelector('.g-dropdown-menu-item')!);
    fireEvent.click(
      container.querySelector(
        '.g-table-filter-dropdown-btns .g-btn-color-primary.g-btn-varig-solid',
  ...
  it('locale should work', () => {
    ...
    expect(
      container.querySelector('.g-table-filter-dropdown-btns .g-btn-link')?.textContent,
    ).toEqual('Reset');
    expect(container.querySelector('.g-table-filter-dropdown-checkall')?.textContent).toEqual(
      'Select all items',
    );
    expect(container.querySelector('.g-input')?.getAttribute('placeholder')).toEqual(
      'Search in filters',
    );
  });
  ...
  it('filtered should work', () => {
    ...
    expect(
      container.querySelector('.g-table-filter-trigger')?.className.includes('active'),
    ).toBeTruthy();
  });
  ...
  it(`shouldn't keep status when controlled filteredValue isn't change`, () => {
    ...
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!); // reopen
    const checkbox = container
      ?.querySelector('.g-dropdown-menu-item')
  ...
  it('filters in children should render', () => {
    ...
    expect(container.querySelectorAll('.g-table-filter-column')).toHaveLength(3);
  });
  ...
  it('should pagination.current be 1 after filtering', () => {
    ...
    fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
    fireEvent.click(container.querySelectorAll('.g-dropdown-menu-item')[1]!);
    fireEvent.click(
      container.querySelector(
        '.g-table-filter-dropdown-btns .g-btn-color-primary.g-btn-varig-solid',
      )!,
  ...
  it('Column with filter and children filters properly.', () => {
    ...
    expect(container.querySelector('.g-table-tbody .g-table-cell')?.textContent).toEqual(`${32}`);
    fireEvent.click(container.querySelector('.g-dropdown-trigger.g-table-filter-trigger')!);
    fireEvent.click(container.querySelector('.g-dropdown-menu-item')!);
    fireEvent.click(
      container.querySelector('.g-btn.g-btn-color-primary.g-btn-varig-solid.g-btn-sm')!,
    );
    expect(container.querySelector('.g-table-tbody .g-table-cell')?.textContent).toEqual(`${66}`);
  });
  ...
  it('Columns with filters should filter correctly after reset it.', () => {
    ...
    expect(container.querySelectorAll('.g-table-tbody .g-table-row').length).toEqual(4);
    // Open
    fireEvent.click(container.querySelector('.g-table-filter-trigger')!);
    function getFilterMenu() {
      return container.querySelector('.g-table-filter-dropdown');
    ...
    expect(container.querySelectorAll('.g-table-tbody .g-table-row').length).toEqual(1);
  ...
  describe('filter tree mode', () => {
    it('supports filter tree', () => {
      ...
      expect(container.querySelectorAll('.g-table-filter-dropdown-tree').length).toBe(1);
      expect(container.querySelectorAll('.g-tree-checkbox').length).toBe(5);
    });
    ...
    it('supports search input in filter tree', () => {
      ...
      fireEvent.change(container.querySelector('.g-input')!, { target: { value: '111' } });
    });
    ...
    it('renders empty element when search not found', () => {
      ...
      expect(container.querySelectorAll('.g-table-filter-dropdown-search').length).toBe(1);
      expect(container.querySelectorAll('.g-input').length).toBe(1);
      fireEvent.change(container.querySelector('.g-input')!, { target: { value: '111' } });
      expect(container.querySelector('.g-empty')).toBeTruthy();
    ...
    it('should skip search when filters[0].text is ReactNode', () => {
      ...
      expect(container.querySelectorAll('.g-table-filter-dropdown-search').length).toBe(1);
      expect(container.querySelectorAll('.g-input').length).toBe(1);
      expect(container.querySelectorAll('li.g-dropdown-menu-item').length).toBe(3);
      fireEvent.change(container.querySelector('.g-input')!, { target: { value: '123' } });
      expect(container.querySelectorAll('li.g-dropdown-menu-item').length).toBe(2);
    ...
    it('should supports filterSearch has type of function', () => {
      ...
      expect(container.querySelectorAll('.g-input').length).toBe(1);
      expect(container.querySelectorAll('li.g-dropdown-menu-item').length).toBe(3);
      fireEvent.change(container.querySelector('.g-input')!, { target: { value: '456' } });
      expect(container.querySelectorAll('li.g-dropdown-menu-item').length).toBe(2);
    ...
    it('should supports filterSearch has type of function when filterMode is tree', () => {
      ...
      expect(container.querySelectorAll('.g-table-filter-dropdown-tree').length).toBe(1);
      expect(container.querySelectorAll('.g-input').length).toBe(1);
      fireEvent.change(container.querySelector('.g-input')!, { target: { value: '节点二' } });
      expect(container.querySelectorAll('.g-tree-treenode.filter-node').length).toBe(1);
    });
    ...
    it('supports check all items', () => {
      ...
      expect(container.querySelectorAll('.g-table-filter-dropdown-checkall').length).toBe(1);
      expect(container.querySelector('.g-table-filter-dropdown-checkall')?.textContent).toBe(
        'Select all items',
      );
      expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(0);

      // fireEvent.change(container.querySelector('.g-table-filter-dropdown-checkall input'), { target: { checked: true } });
      // 为什么 fireEvent.change 模拟 checkbox 触发会失败
      fireEvent.click(container.querySelector('.g-table-filter-dropdown-checkall')!);
      expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(5);
    ...
    it('supports check item by selecting it', () => {
      ...
      expect(
        container?.querySelector('.g-tree-checkbox')?.className.includes('g-tree-checkbox-checked'),
      ).toBe(true);
      expect(
        container
          ?.querySelector('.g-table-filter-dropdown-checkall .g-checkbox')
          ?.className.includes('g-checkbox-indeterminate'),
      ).toBe(true);
    });
    ...
    it('select-all checkbox should change when all items are selected', () => {
      ...
      expect(
        container
          ?.querySelector('.g-table-filter-dropdown-checkall .g-checkbox')
          ?.className.includes('g-checkbox-checked'),
      ).toBe(true);
    });
  });
  ...
  it('filterMultiple is false - check item', () => {
    ...
    expect(container.querySelectorAll('.g-tree-checkbox').length).toBe(5);
    expect(container.querySelector('.g-table-filter-dropdown-checkall')).toBe(null);
    expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(0);

    fireEvent.click(container.querySelectorAll('.g-tree-checkbox')[2]);
    expect(
      container
        .querySelectorAll('.g-tree-checkbox')[2]
        .className.includes('g-tree-checkbox-checked'),
    ).toBe(true);
    expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(1);

    fireEvent.click(container.querySelectorAll('.g-tree-checkbox')[1]);
    expect(
      container
        .querySelectorAll('.g-tree-checkbox')[1]
        .className.includes('g-tree-checkbox-checked'),
    ).toBe(true);
    expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(1);
  ...
  it('filterMultiple is false - select item', () => {
    ...
    fireEvent.click(container.querySelectorAll('.g-tree-node-content-wrapper')[2]);
    expect(
      container
        .querySelectorAll('.g-tree-checkbox')[2]
        .className.includes('g-tree-checkbox-checked'),
  ...
  it('should select children when select parent', () => {
    ...
    expect(
      container
        .querySelectorAll('.g-tree-checkbox')[3]
        .className.includes('g-tree-checkbox-checked'),
    ).toBe(true);
    expect(
      container
        .querySelectorAll('.g-tree-checkbox')[4]
        .className.includes('g-tree-checkbox-checked'),
    ).toBe(true);
    ...
    fireEvent.click(container.querySelectorAll('.g-tree-checkbox-inner')[2]);
    fireEvent.click(
      container.querySelector(
        '.g-table-filter-dropdown-btns .g-btn-color-primary.g-btn-varig-solid',
      )!,
  ...
  it('clearFilters should support params', () => {
    ...
    filterConfig.forEach(([text, id, , matchNames, visible]) => {
      ...
      expect(container.querySelector('.g-dropdown-open'))[visible ? 'toBeTruthy' : 'toBeFalsy']();
    });
  });
  ...
  it('filterDropdown should support filterResetToDefaultFilteredValue', () => {
    ...
    fireEvent.click(container.querySelector('.g-table-filter-dropdown-checkall')!);
    expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(5);
    fireEvent.click(container.querySelector('button.g-btn-link')!, nativeEvent);
    expect(container.querySelectorAll('.g-tree-checkbox-checked').length).toBe(0);
    ...
    fireEvent.click(container2.querySelector('span.g-dropdown-trigger')!, nativeEvent);
    act(() => {
      jest.runAllTimers();
    });
    fireEvent.click(container2.querySelector('.g-table-filter-dropdown-checkall')!);
    expect(container2.querySelectorAll('.g-tree-checkbox-checked').length).toBe(5);
    fireEvent.click(container2.querySelector('button.g-btn-link')!, nativeEvent);
    expect(container2.querySelectorAll('.g-tree-checkbox-checked').length).toBe(1);
    expect(container2.querySelector('.g-tree-checkbox-checked+span')?.textContent).toBe('Girl');
  });
  ...
  it('filterDropdown should not override customize Menu selectable', () => {
    ...
    // Click Item
    fireEvent.click(container.querySelector('.g-table-filter-dropdown .g-dropdown-menu-item')!);
  ...
  it('can reset if filterResetToDefaultFilteredValue and filter is changing', () => {
    ...
    fireEvent.click(container.querySelector('span.g-dropdown-trigger')!);
    expect(
      container.querySelector<HTMLLinkElement>('.g-table-filter-dropdown-btns .g-btn-link')
        ?.disabled,
    ).toBeTruthy();
    expect(container.querySelectorAll('li.g-dropdown-menu-item')[0].textContent).toBe('Jack');
    expect(container.querySelectorAll('li.g-dropdown-menu-item')[1].textContent).toBe('Lucy');

    // deselect default
    fireEvent.click(container.querySelectorAll('li.g-dropdown-menu-item')[0]);
    expect(
      container.querySelector<HTMLLinkElement>('.g-table-filter-dropdown-btns .g-btn-link')
        ?.disabled,
    ).toBeFalsy();
  ...
  it('title render function support `filter`', () => {
    ...
    expect(container.querySelector('.g-table-column-title')?.textContent).toEqual('RenderTitle');
  ...
  describe('empty state', () => {
    ...
    it('should return custom content', async () => {
      ...
      expect(container.querySelector('.g-table-filter-dropdown')).toHaveTextContent('foo');
    ...
    it('allow `false` to not render empty states', async () => {
      ...
      expect(container.querySelector('.g-table-filter-dropdown .g-empty')).toBeNull();
      expect(container.querySelector('.g-table-filter-dropdown')!.childNodes).toHaveLength(1);
    });
  });
  ...
  describe('should support filterDropdownProps', () => {
    ...
    it('placement', () => {
      ...
      fireEvent.click(container.querySelector('.g-dropdown-trigger')!);
      expect(container.querySelector('.g-dropdown-placement-topLeft')).toBeTruthy();
    });
  });
});
