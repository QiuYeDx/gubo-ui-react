...
describe('Transfer.Dropdown', () => {
  function clickItem(container: HTMLElement, index: number) {
    const items = Array.from(
      container
        ...
        // Items
        .querySelectorAll('li.g-dropdown-menu-item'),
    );
  ...
  it('should hide checkbox and dropdown icon when showSelectAll={false}', () => {
    ...
    expect(container.querySelector('.g-dropdown-trigger')).toBeFalsy();
    expect(
      container.querySelector('.g-transfer-list-header .g-transfer-list-checkbox'),
    ).toBeFalsy();
  });
  ...
  describe('oneWay to remove', () => {
    [
      ...
      it(name, () => {
        ...
        // Right dropdown
        fireEvent.mouseEnter(container.querySelectorAll('.g-dropdown-trigger')[1]!);
        act(() => {
          jest.runAllTimers();
        });
